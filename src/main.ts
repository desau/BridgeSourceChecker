...
import { saveAllErrors, scanErrors, getErrorText, printFolderRenames } from './Scanner/ScanErrors'
dotenv.config()
...
async function main() {
  ...
  try {
    const versions = await scanNewDownloads(versionsToScan)
    printFolderRenames(versions)
